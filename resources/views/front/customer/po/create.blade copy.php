...
<body>
    ...
    <div class="app-content style-3">
        <div class="tf-container">
            <form action="successfully-applied.html" class="mt-10">
                <div class="form-field form-2">
                    <div class="label h7">Pilih Gramasi Emas</div>
                    <fieldset class="mt-12">
                         <select class="form-control" name="produk_id">
                            @foreach($produkPo as $p)
                                <option value="{{ $p->id }}" data-harga="{{ (float)($p->harga_hariini ?? 0) }}">{{ optional($p->gramasi)->nama }} - {{ number_format((float)optional($p->gramasi)->gramasi, 3, ',', '.') }} gram</option>
                            @endforeach
                        </select>
                    </fieldset>
                </div>
    ...
    <script>
        (function($){
            ...
            function updateEstimasi(){
                var $prod = $('select[name="produk_id"]');
                var harga = Number($prod.find('option:selected').data('harga')) || 0;
            ...
            $(document).ready(function(){
                $('select[name="produk_id"], #qty').on('change', updateEstimasi);
                updateEstimasi();
            });
        })(jQuery);
    </script>
