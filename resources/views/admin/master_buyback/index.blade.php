...
@section('title', 'Master Buyback - Admin')
@section('sub-title', 'Buyback')
@section('breadcrumbExtra', 'Master Buyback')
@section('pagetitle', 'Dashboard')
@section('subLink', route('admin.master.buyback.index'))

@section('content')
    <div class="card shadow-sm">
        <table id="buybackTable" class="data-table-added table-hover align-middle table table-nowrap w-100">
            ...
            <tbody>
                @forelse ($items as $it)
                    <tr>
                        ...
                        <td class="text-uppercase">{{ $it->brand }}</td>
                        <td>{{ $it->berat }}</td>
                        <td class="text-end">Rp {{ number_format((float)$it->harga, 0, ',', '.') }}</td>
                        <td class="text-end fw-semibold text-success">Rp {{ number_format((float)$it->buyback, 0, ',', '.') }}</td>
                        <td>{{ $it->stok }}</td>
                        <td class="text-center"><span class="badge rounded-pill bg-light text-dark border">{{ $it->status }}</span></td>
                        <td>
                            <div class="hstack gap-2 fs-15">
                                <a href="{{ route('admin.master.buyback.edit', $it->id) }}" class="btn icon-btn-sm btn-light-primary">
                                    <i class="ri-pencil-line"></i>
                                </a>
                                <a href="#" class="btn icon-btn-sm btn-light-danger delete-item"
                                    data-action="{{ route('admin.master.buyback.destroy', $it->id) }}"
                                    data-label="{{ $it->brand ? $it->brand : ('#' . $it->id) }}">
                                    <i class="ri-delete-bin-line"></i>
                                </a>
                            </div>
                            <form action="{{ route('admin.master.buyback.destroy', $it->id) }}" method="POST" class="d-none delete-form">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const tableEl = document.getElementById('buybackTable');
            if (!tableEl) return;
            if (typeof $ === 'undefined' || !$.fn.DataTable) return;

            const dt = $('#buybackTable').DataTable({
                responsive: false,
                scrollX: true,
                lengthMenu: [10, 20, 50],
                pageLength: 10,
            ...
            const savedPage = sessionStorage.getItem('buybackTablePage');
            if (savedPage !== null) {
                const p = parseInt(savedPage, 10);
                if (!Number.isNaN(p)) {
                    dt.page(p).draw('page');
                }
                sessionStorage.removeItem('buybackTablePage');
            }

            const headLabel = document.querySelector('div.head-label');
            if (headLabel) {
                headLabel.innerHTML = '<h5 class="card-title text-nowrap mb-0">Daftar Buyback</h5>';
            }

            const addBtnContainer = document.querySelector('.add_button');
            if (addBtnContainer) {
                addBtnContainer.innerHTML = '<a class="btn btn-primary" href="{{ route('admin.master.buyback.create') }}"><i class="bi bi-plus-lg fs-6 me-1"></i> Tambah Data</a>';
            }

            document.querySelectorAll('.delete-item').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const label = this.getAttribute('data-label') || 'Buyback';
                    const row = this.closest('tr');
                    const form = row ? row.querySelector('form.delete-form') : null;
                    if (!form) return;
                    Swal.fire({
                        title: 'Konfirmasi Hapus',
                        html: `Anda yakin hapus Buyback <b>${label}</b> ini?`,
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                sessionStorage.setItem('buybackTablePage', String(info.page));
                            } catch (_) {}
                            form.submit();
                        }
                    });
