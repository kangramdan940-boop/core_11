...
@section('title', 'Trans Cicilan Emas - Admin')
...
@section('content')
    <div class="card shadow-sm">
        <table id="cicilanEmasTable" class="data-table-added table-hover align-middle table table-nowrap w-100">
            ...
            <tbody>
                @forelse ($items as $it)
                    <tr>
                        ...
                        <td>{{ optional($it->layanan)->kode_layanan }} — {{ optional($it->layanan)->nama_layanan }}</td>
                        <td>{{ optional($it->agen)->name }}</td>
                        <td class="text-end">
                            {{ optional($it->gramasi)->nama }}
                            ({{ number_format((float)optional($it->gramasi)->gramasi, 0, ',', '.') }} g)
                        </td>
                        <td class="text-end">{{ (int)$it->jumlah_keping_dibuka }}</td>
                        <td class="text-end">{{ number_format((float)$it->total_gram_dibuka, 3, ',', '.') }} g</td>
                        <td>
                            <div class="hstack gap-2 fs-15">
                                <a href="{{ route('admin.trans.cicilan-emas.edit', $it) }}" class="btn icon-btn-sm btn-light-primary">
                                    <i class="ri-pencil-line"></i>
                                </a>
                                <a href="#" class="btn icon-btn-sm btn-light-danger delete-item"
                                   data-action="{{ route('admin.trans.cicilan-emas.destroy', $it) }}"
                                   data-label="{{ '#'.$it->id }}">
                                    <i class="ri-delete-bin-line"></i>
                                </a>
                            </div>
                            <form action="{{ route('admin.trans.cicilan-emas.destroy', $it) }}" method="POST" class="d-none delete-form">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const tableEl = document.getElementById('cicilanEmasTable');
            if (!tableEl) return;
            if (typeof $ === 'undefined' || !$.fn.DataTable) return;

            const dt = $('#cicilanEmasTable').DataTable({
                responsive: false,
                scrollX: true,
                lengthMenu: [10, 20, 50],
                pageLength: 10,
            ...
            if (headLabel) {
                headLabel.innerHTML = '<h5 class="card-title text-nowrap mb-0">Daftar Trans Cicilan Emas</h5>';
            }

            const addBtnContainer = document.querySelector('.add_button');
            if (addBtnContainer) {
                addBtnContainer.innerHTML = '<a class="btn btn-primary" href="{{ route('admin.trans.cicilan-emas.create') }}"><i class="bi bi-plus-lg fs-6 me-1"></i> Tambah Data</a>';
            }
            ...
            document.querySelectorAll('.delete-item').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    ...
                    const label = this.getAttribute('data-label') || 'Record';
                    if (!form) return;
                    Swal.fire({
