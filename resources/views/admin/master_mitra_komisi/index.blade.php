...
@section('breadcrumbExtra', 'Master Komisi Mitra')
    ...
    <div class="card shadow-sm">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table id="komisiTable" class="data-table-added table-hover align-middle table table-nowrap w-100">
                    ...
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="hstack gap-2 fs-15">
                                        <a href="{{ route('admin.master.mitra-komisi.edit', $k) }}" class="btn icon-btn-sm btn-light-primary">
                                            <i class="ri-pencil-line"></i>
                                        </a>
                                        <a href="#" class="btn icon-btn-sm btn-light-danger delete-item"
                                           data-action="{{ route('admin.master.mitra-komisi.destroy', $k) }}"
                                           data-label="{{ optional($k->mitra)->nama_lengkap ?? ('#' . $k->id) }}">
                                            <i class="ri-delete-bin-line"></i>
                                        </a>
                                    </div>
                                    <form action="{{ route('admin.master.mitra-komisi.destroy', $k) }}" method="POST" class="d-none delete-form">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-3">Belum ada data komisi mitra.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if ($komisis->hasPages())
                <div class="p-2">{{ $komisis->links() }}</div>
            @endif
        </div>
    </div>
@endsection
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const tableEl = document.getElementById('komisiTable');
            if (!tableEl) return;
            if (typeof $ === 'undefined' || !$.fn.DataTable) return;

            const dt = $('#komisiTable').DataTable({
                responsive: false,
                scrollX: true,
                lengthMenu: [10, 20, 50],
                pageLength: 10,
            ...
            const savedPage = sessionStorage.getItem('komisiTablePage');
            if (savedPage !== null) {
                const p = parseInt(savedPage, 10);
                if (!Number.isNaN(p)) {
                    dt.page(p).draw('page');
                }
                sessionStorage.removeItem('komisiTablePage');
            }

            const headLabel = document.querySelector('div.head-label');
            if (headLabel) {
                headLabel.innerHTML = '<h5 class="card-title text-nowrap mb-0">Daftar Komisi Mitra</h5>';
            }

            const addBtnContainer = document.querySelector('.add_button');
            if (addBtnContainer) {
                addBtnContainer.innerHTML = '<a class="btn btn-primary" href="{{ route('admin.master.mitra-komisi.create') }}"><i class="bi bi-plus-lg fs-6 me-1"></i> Tambah Data</a>';
            }

            document.querySelectorAll('.delete-item').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const label = this.getAttribute('data-label') || 'Komisi';
                    const row = this.closest('tr');
                    const form = row ? row.querySelector('form.delete-form') : null;
                    if (!form) return;
                    Swal.fire({
                        title: 'Konfirmasi Hapus',
                        html: `Anda yakin hapus Komisi <b>${label}</b> ini?`,
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                sessionStorage.setItem('komisiTablePage', String(info.page));
                            } catch (_) {}
                            form.submit();
                        }
                    });
