...
@section('breadcrumbExtra', 'Master Agen')
    ...
    <div class="card shadow-sm">
        <div class="card-body p-0">
            <div class="table-responsive">
                <table id="agenTable" class="data-table-added table-hover align-middle table table-nowrap w-100">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="hstack gap-2 fs-15">
                                        <a href="{{ route('admin.master.agens.edit', $a) }}" class="btn icon-btn-sm btn-light-primary">
                                            <i class="ri-pencil-line"></i>
                                        </a>
                                        <a href="#" class="btn icon-btn-sm btn-light-danger delete-item"
                                           data-action="{{ route('admin.master.agens.destroy', $a) }}"
                                           data-label="{{ $a->name ? $a->name : ('#' . $a->id) }}">
                                            <i class="ri-delete-bin-line"></i>
                                        </a>
                                    </div>
                                    <form action="{{ route('admin.master.agens.destroy', $a) }}" method="POST" class="d-none delete-form">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const tableEl = document.getElementById('agenTable');
            if (!tableEl) return;
            if (typeof $ === 'undefined' || !$.fn.DataTable) return;

            const dt = $('#agenTable').DataTable({
                responsive: false,
                scrollX: true,
                lengthMenu: [10, 20, 50],
                pageLength: 10,
            ...
            const savedPage = sessionStorage.getItem('agenTablePage');
            if (savedPage !== null) {
                const p = parseInt(savedPage, 10);
                if (!Number.isNaN(p)) {
                    dt.page(p).draw('page');
                }
                sessionStorage.removeItem('agenTablePage');
            }

            const headLabel = document.querySelector('div.head-label');
            if (headLabel) {
                headLabel.innerHTML = '<h5 class="card-title text-nowrap mb-0">Daftar Agen</h5>';
            }

            const addBtnContainer = document.querySelector('.add_button');
            if (addBtnContainer) {
                addBtnContainer.innerHTML = '<a class="btn btn-primary" href="{{ route('admin.master.agens.create') }}"><i class="bi bi-plus-lg fs-6 me-1"></i> Tambah Data</a>';
            }

            document.querySelectorAll('.delete-item').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const label = this.getAttribute('data-label') || 'Agen';
                    const row = this.closest('tr');
                    const form = row ? row.querySelector('form.delete-form') : null;
                    if (!form) return;
                    Swal.fire({
                        title: 'Konfirmasi Hapus',
                        html: `Anda yakin hapus Agen <b>${label}</b> ini?`,
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                sessionStorage.setItem('agenTablePage', String(info.page));
                            } catch (_) {}
                            form.submit();
                        }
                    });
