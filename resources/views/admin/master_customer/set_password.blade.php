...
@section('content')
    <div class="card shadow-sm">
        <div class="card-header">
            <h5 class="mb-0">Set Password Sys User Customer</h5>
        </div>
        <div class="card-body">
            <div class="mb-3">
            ...
            <form action="{{ route('admin.master.customers.set-password.update', $customer) }}" method="POST" class="row g-3">
                @csrf
                <div class="col-12 col-md-6">
                    <label class="form-label" for="Email">Email Login</label>
                    <input type="email" id="Email" name="email" class="form-control" required maxlength="150" value="{{ old('email', $user->email ?? $customer->email) }}" autocomplete="email">
                </div>
                <div class="col-12 col-md-6">
                ...
                <div class="col-12 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.master.customers.index') }}" class="btn btn-outline-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
