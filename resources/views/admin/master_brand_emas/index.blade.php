...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const tableEl = document.getElementById('brandsTable');
            if (!tableEl) return;
            ...
            const dt = $('#brandsTable').DataTable({
                ...
                dom:
                    ...
                    '<"row w-100 align-items-center g-2"' +
                        '<"col-12 col-md-8 d-flex align-items-center justify-content-md-start justify-content-center gap-1"l i>' +
            ...
            if (headLabel) {
                headLabel.innerHTML = '<h5 class="card-title text-nowrap mb-0">Daftar Brand Emas</h5>';
            }

            const addBtnContainer = document.querySelector('.add_button');
            if (addBtnContainer) {
                addBtnContainer.innerHTML = '<a class="btn btn-primary" href="{{ route('admin.master.brand-emas.create') }}">Add Data</a>';
            }

            const savedPage = sessionStorage.getItem('brandsTablePage');
            if (savedPage !== null) {
                const p = parseInt(savedPage, 10);
                if (!Number.isNaN(p)) {
                    dt.page(p).draw('page');
                }
                sessionStorage.removeItem('brandsTablePage');
            }

            document.querySelectorAll('.delete-item').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const label = this.getAttribute('data-label') || 'Brand Emas';
                    const row = this.closest('tr');
                    const form = row ? row.querySelector('form.delete-form') : null;
                    if (!form) return;
                    Swal.fire({
                        title: 'Konfirmasi Hapus',
                        html: `Anda yakin hapus Brand <b>${label}</b> ini?`,
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                sessionStorage.setItem('brandsTablePage', String(info.page));
                            } catch (_) {}
                            form.submit();
                        }
                    });
            ...
            document.addEventListener('click', function(e) {
                const anchor = e.target.closest('.zoomable-thumb');
                if (!anchor) return;
                e.preventDefault();
                const src = anchor.getAttribute('data-src');
                const label = anchor.getAttribute('data-label') || 'Preview';
                Swal.fire({
                    title: label,
                    imageUrl: src,
                    imageAlt: label,
