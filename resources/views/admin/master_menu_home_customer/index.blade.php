...
@section('breadcrumbExtra', 'Master Menu Home Customer')
...
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const tableEl = document.getElementById('menuHomeTable');
            if (!tableEl) return;
            if (typeof $ === 'undefined' || !$.fn.DataTable) return;

            const dt = $('#menuHomeTable').DataTable({
                responsive: false,
                scrollX: true,
                lengthMenu: [10, 20, 50],
                pageLength: 10,
            ...
            const savedPage = sessionStorage.getItem('menuHomeTablePage');
            if (savedPage !== null) {
                const p = parseInt(savedPage, 10);
                if (!Number.isNaN(p)) {
                    dt.page(p).draw('page');
                }
                sessionStorage.removeItem('menuHomeTablePage');
            }

            const headLabel = document.querySelector('div.head-label');
            if (headLabel) {
                headLabel.innerHTML = '<h5 class="card-title text-nowrap mb-0">Daftar Menu Home Customer</h5>';
            }

            const addBtnContainer = document.querySelector('.add_button');
            if (addBtnContainer) {
                addBtnContainer.innerHTML = '<a class="btn btn-primary" href="{{ route('admin.master.menu-home-customer.create') }}"><i class="bi bi-plus-lg fs-6 me-1"></i> Tambah Data</a>';
            }
            ...
            document.querySelectorAll('.delete-item').forEach(function(btn) {
                btn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const label = this.getAttribute('data-label') || 'Menu';
                    const row = this.closest('tr');
                    const form = row ? row.querySelector('form.delete-form') : null;
                    if (!form) return;
                    Swal.fire({
                        title: 'Konfirmasi Hapus',
                        html: `Anda yakin hapus Menu <b>${label}</b> ini?`,
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                sessionStorage.setItem('menuHomeTablePage', String(info.page));
                            } catch (_) {}
                            form.submit();
                        }
                    });
